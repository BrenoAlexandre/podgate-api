...
export class AddToFavoritesUseCase {
  ...
  private async validate(feedId: string) {
    ...
    if (errors.length > 0) {
      throw CustomError.badRequest('Unable to add feed to favorites', errors);
    }
  }
  ...
  public async execute(data: {
    ...
    if (!hasList) {
      const newList = await this.favoriteRepository.create(userId, feedId);
      if (!newList) {
    ...
    const newList = await this.favoriteRepository.addToList(userId, feedId);

    if (!newList) {
      throw CustomError.badRequest('Unable to add to favorites.');
    }
